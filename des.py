...
class des():
    ...
    def compute_s_box(self, block,round):
        ...
        if ((block[-1]==1) & (block[0]==1)):
            ...
            row=2
        elif ((block[-1]==1) & (block[0]==0)):
            row=1
        else:
            row=0
        block=block[1:-1]
        block_fin = "".join(str(x) for x in block)
        column=int(block_fin,2)
        temp = S_BOX[round-1][row][column]
        bin = binvalue(temp,4)
        return bin
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        for i in range(0,len(subblocks)):
            templist.append(self.compute_s_box(subblocks[i],i+1)) 
        result =  self.func(templist)
        return result
